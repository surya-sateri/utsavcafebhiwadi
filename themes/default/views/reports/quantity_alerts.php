...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-calendar-o"></i><?= lang('product_quantity_alerts') . ' (' . (!empty($warehouse_id) && is_numeric($warehouse_id) ? $warehouse[$warehouse_id]->name : lang('all_warehouses')) . ')'; ?>
                 <?php // lang('product_quantity_alerts') . ' (' . ($warehouse_id ? (isset($warehouse->name)?$warehouse->name:lang('all_warehouses')) : lang('all_warehouses')) . ')'; ?>
        </h2>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#image').click(function (event) {
            event.preventDefault();
			window.location.href = "<?=site_url('reports/getQuantityAlerts/'.($warehouse_id ? $warehouse_id : '0').'/0/0/img')?>";
           /* html2canvas($('.box'), {
