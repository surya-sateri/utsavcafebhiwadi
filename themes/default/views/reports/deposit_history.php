...
<script>
    $(document).ready(function () {
        var oTable = $('#SlRData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('reports/getDepositHistory/?v=1'. $v) ?>',
            ...
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                ...
                var date = nRow.date; 
                console.log(aData[9]);
                 var url = '<?= site_url("Customers/getDepositreacharge") ?>?date='+date+'&customer_id='+aData[9];
                    $.ajax({
                    type:'ajax',
                    dataType:'json',
                    url:url,
                    ...
                    success:function(result){
                        console.log(result);
                        var closingBlaance = (parseFloat(aData[5] ) + parseFloat(result.amount)) - parseFloat(result.useddeposit); 
                        nCells[6].innerHTML =  currencyFormat(result.amount);
                        nCells[7].innerHTML =  currencyFormat(result.useddeposit);
                        nCells[8].innerHTML =  currencyFormat(closingBlaance);
                      
                    }, error:function(){
                         nCells[6].innerHTML = currencyFormat(0);
                        nCells[7].innerHTML = currencyFormat(0);
            ...
            "aoColumns": [null,null, null, null,null, {"mRender": currencyFormat},  {"mRender": currencyFormat},{"mRender": currencyFormat},{"mRender": currencyFormat},{"bSearchable": false,"bVisible":false}
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('Deposit Report'); ?> <?php
            if ($this->input->post('start_date')) {
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo form_open("reports/depositHistory"); ?>
                    <div class="row">
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('reports/getDepositHistory/pdf/?v=1'.$v)?>";
            return false;
         });

        $('#xls').click(function (event) {         
            event.preventDefault();          
            window.location.href = "<?=site_url('reports/getDepositHistory/0/xls/?v=1'.$v)?>";
            return false;
        });

        $('#image').click(function (event) {
            event.preventDefault();
			  window.location.href = "<?=site_url('reports/getDepositHistory/0/0/img/?v=1'.$v)?>";
