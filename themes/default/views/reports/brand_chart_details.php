...
<?php if ($Owner || $Admin) { ?>
    <div class="box" style="margin-top: 15px;">
                    ...
                    <?php if (!empty($warehouses) && ($Owner || $Admin)) { ?>
                        <li class="dropdown">
                            ...
                            <ul class="dropdown-menu pull-right tasks-menus" role="menu"
                                ...
                                <li><a href="<?= site_url('reports/brand_chart_details') ?>"><i
                                            class="fa fa-building-o"></i> <?= lang('all_warehouses') ?></a></li>
                                <li class="divider"></li>
                                <?php
                                foreach ($warehouses as $warehouse) {
                                    echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' . site_url('reports/brand_chart_details/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                                }
                                ?>
                            </ul>
                        </li>
		...
		<div class="row">
				<div class="col-lg-12">
					<h1 class="introtext" style="text-align:center;"><?php echo lang('Monthly Brand Chart Details'); ?></h1>
			<?php foreach($months as $km =>$kval){ if(!empty($monthly_arr[$kval])){ ?>
