...
<div class="box">
    <div class="box-header">
                <h2 class="blue"><i class="fa-fw fa fa-calendar"></i><?= lang('Urban Piper Daily Sales').' ('.($sel_warehouse ? isset($sel_warehouse[$this->uri->segment(3)]->name)?$sel_warehouse[$this->uri->segment(3)]->name: lang('all_warehouses'): lang('all_warehouses')).')'; ?></h2>
                ...
                <?php // if (!empty($warehouses) && !$this->session->userdata('warehouse_id')) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            ...
                            <li><a href="<?=site_url('reports/daily_sales_up/'.$allwarehouse.'/'.$year.'/'.$month)?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            ...
                            <?php
                               $permisions_werehouse = explode(",", $warehouse_id);
                                foreach ($warehouses as $warehouse) {
                                    if($Owner || $Admin   ){
                                        echo '<li><a href="' . site_url('reports/daily_sales_up/'.$warehouse->id.'/'.$year.'/'.$month) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#pdf').click(function (event) {
           ...
           // window.location.href = "<?php //site_url('reports/daily_sales/'.($sel_warehouse ? $sel_warehouse->id : 0).'/'.$year.'/'.$month.'/pdf')?>"; // Problem For calander
            window.location.href = "<?=site_url('reports/daily_sales/'.( ($this->uri->segment(3))? $this->uri->segment(3) : 0).'/'.$year.'/'.$month.'/pdf')?>"; // Update 9-9-19
            return false;
        });
         $('#xls').click(function (event) {
           ...
           // window.location.href = "<?php //site_url('reports/daily_sales/'.($sel_warehouse ? $sel_warehouse->id  : 0).'/'.$year.'/'.$month.'/xls')?>"; // Problem For calander
             window.location.href = "<?=site_url('reports/daily_sales/'.(($this->uri->segment(3))? $this->uri->segment(3) : 0).'/'.$year.'/'.$month.'/xls')?>"; // Update 9-9-19
            return false;
        });
	...
	//    window.location.href = "<?php //site_url('reports/daily_sales/'.($sel_warehouse ? $sel_warehouse->id  : 0).'/'.$year.'/'.$month.'/img')?>"; // Problem For calander
	    window.location.href = "<?=site_url('reports/daily_sales/'.(($this->uri->segment(3))? $this->uri->segment(3) : 0).'/'.$year.'/'.$month.'/img')?>"; // Update 9-9-19
	...
	var postData = 'date=' + date+'&active_warehouse_id='+active_warehouse_id;
          ...
          var href = '<?= site_url('reports/daily_sales_items'); ?>?'+postData;
            $.get(href, function( data ) {
	...
	var postData = 'date=' + date+'&active_warehouse_id='+active_warehouse_id;
          ...
          var href = '<?= site_url('reports/daily_sales_items_taxes'); ?>?'+postData;
            $.get(href, function( data ) {
                $("#model_body").html(data);
            });
     ...
     function printdiv(date){ 
        ...
        var href = '<?= site_url('reports/daily_sales_items_print'); ?>?'+postData;
            $.get(href, function( data ) {
