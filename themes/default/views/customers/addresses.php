...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
                    ...
                    <?php if (!empty($addresses)) {
                        foreach ($addresses as $address) {
                            echo '<tr>' .
                                ...
                                '<td class="text-center"><a href="' . site_url('customers/edit_address/' . $address->id) . '" class="tip" title="' . lang("edit_address") . '" data-toggle="modal" data-target="#myModal2"><i class="fa fa-edit"></i></a> <a href="#" class="tip po" title="' . $this->lang->line("delete_address") . '" data-content="<p>' . lang('r_u_sure') . '</p><a class=\'btn btn-danger\' href=\'' . site_url('customers/delete_address/' . $address->id) . '\'>' . lang('i_m_sure') . '</a> <button class=\'btn po-close\'>' . lang('no') . '</button>"  rel="popover"><i class="fa fa-trash-o"></i></a></td>' .
                                '</tr>';
                        }
                    } else { ?>
                        <tr>
        ...
        <div class="modal-footer">
            <a href="<?= site_url('customers/add_address/'.$company->id); ?>" class="btn btn-primary pull-left" data-toggle='modal' data-target='#myModal2'><?= lang('add_address'); ?></a>
