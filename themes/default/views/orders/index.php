...
<script>
    $(document).ready(function () {
        var oTable = $('#SLData').dataTable({
            ...
            'sAjaxSource': '<?=site_url('orders/getOrders' . $warehouseIds )?>',
        ...
        $(document).on('click', '.sledit', function (e) {
            if (localStorage.getItem('slitems')) {
                ...
                bootbox.confirm("<?=lang('you_will_loss_order_data')?>", function (result) {
                    if (result) {
...
<?php if ($Owner || $GP['bulk_actions']) {
	    echo form_open('orders/order_actions', 'id="action-form"');
	}
?>
<div class="box">
    <div class="box-header">
        <h2 class="blue">
            <i class="fa-fw fa fa-heart"></i><?=lang('orders') . ' (' . (!empty($warehouse_id) && is_numeric($warehouse_id) ? $warehouse[$warehouse_id]->name : lang('all_warehouses')) . ')';?>
        </h2>
