                ...
                <div id="receipt-data" style="font-size: <?php echo $font_size; ?>pt !important">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <div class="pull-left"><b><?php echo lang("Invoice No.") . "</b>:&nbsp" . $inv->invoice_no; ?>
                    ...
                    <?php
                       /* echo '<b>'.lang("customer") . "</b>: " . $inv->customer ;
                        ...
                        ?>
                                        ...
                                        <div class="row">
                            <?php if (isset($shipping_details) && $inv->order_no) { ?>
                            <div class="col-sm-6">    
                                <?php
                            ...
                            <div class="col-sm-6">
                                ...
                                <address>
                                    ...
                                    <?php
                                    if ($shipping_details) {
                                        echo $shipping_details->shipping_method_name .'&nbsp;';
                                    }if ($shipping_details->deliver_later) {
                                        echo ' <b>Date</b> : ' . date('d-m-Y', strtotime($shipping_details->deliver_later)).'&nbsp;';
...
<?php } ?>
                    ...
                    <div style="clear:both;"></div> 
                        ...
                        if ($payments) {
                            ...
                            echo '<div style="margin-top:-5px;" ><table class="table" style="margin-bottom:0px;" ><tbody>';

                            foreach ($payments as $payment) {
                                
                                $getrounding = salesRounding($payment->sale_id);   
                                ...
                                if ($payment->paid_by == 'cash' || $payment->paid_by == 'deposit' || $payment->paid_by == 'Due Payment') {
                                    ...
                                    if ($payments[count($payments) - 1]->paid_by == 'cash') {
                                       // echo '<td align="right">' . (($payment->pos_balance >= '0.0000') ? lang("balance") . ':&nbsp;' : lang("change") . ':&nbsp;') . $this->sma->formatMoney($payment->pos_balance) . '</td>';
                                       
                                     
                                     if($inv->pos==1){
										if ($payment->pos_balance >= 0) {
											echo '<td align="right">Balance :'.($payment->type=='returned' ? $payment->pos_balance  : $this->sma->formatMoney($payment->pos_paid  - ($inv->grand_total + $rounding))) . '</td>';
										 }else{
											echo '<td align="right">Balance :'.($payment->type=='returned' ? $payment->pos_balance  : $this->sma->formatMoney($payment->pos_paid  - ($inv->grand_total + $rounding))) . '</td>';
										  }
									 }else{
										 $PaymentPaidAmt+=$payment->amount;
										 echo '<td align="right">Change :'.($payment->type=='returned' ? $payment->amount  : $this->sma->formatMoney(($inv->grand_total + $rounding)-$PaymentPaidAmt)) . '</td>';
									 }
                                        //echo '<td align="right">' . (($payment->pos_balance >= 0) ? lang("balance") . ':&nbsp;' : lang("change") . ':&nbsp;') .($payment->type=='returned')? $payment->pos_balance  : $this->sma->formatMoney($payment->pos_paid  - ($inv->grand_total + $rounding)) . '</td>';
                                ...
                                } elseif (($payment->paid_by == 'other' || $payment->paid_by == 'NEFT' || $payment->paid_by == 'PAYTM' || $payment->paid_by == 'Googlepay' || $payment->paid_by == 'complimentry' || $payment->paid_by == 'swiggy' || $payment->paid_by == 'zomato' || $payment->paid_by == 'ubereats' || $payment->paid_by == 'magicpin'|| $payment->paid_by == 'UPI_QRCODE') && $payment->amount) {
                                    echo '<td>' . lang("paid_by") . ':&nbsp;' . lang($payment->paid_by) . '</td>';
                                    echo '<td align="center">Transaction No : ' . $payment->transaction_id . '</td>';
                                    echo '<td align="center">Paid ' . lang("amount") . ':&nbsp;' . $this->sma->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                ...
                if ($modal) {
                    ...
                    <div id="buttons" style="padding-top:10px; text-transform:uppercase;" class="no-print">
                       ...
                       <!-- <span class="col-xs-12">
                        ...
                        <span class="col-xs-12">
                            <a class="btn btn-block btn-warning"  href="<?= $go_back; ?>"><?= lang("GO_BACK"); ?></a>
                        </span>
                        <?php if (!$pos_settings->java_applet) { ?>
                            <div style="clear:both;"></div>
