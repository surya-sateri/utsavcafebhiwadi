...
class Auth_model extends CI_Model
{
   ...
   public function getGroup($id = NULL){
    ...
    {
        $sql = 'SELECT users.`id`, users.`username`, users.`email`, users.`first_name`, users.`last_name`, users.`company`, users.`phone`, users.`gender`, users.`group_id`, groups.`name` as user_type, users.`warehouse_id`, warehouses.`name` as warehouse, users.`biller_id`, companies.`name` as biller, users.`company_id`, users.`show_cost`, users.`show_price`, users.`award_points`, users.`view_right`, users.`edit_right`, users.`allow_discount`, users.`active`, users.`offline_mobile_app_access`, users.`offline_windows_app_access` '; 
