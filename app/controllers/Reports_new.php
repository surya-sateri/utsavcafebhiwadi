    ...
    private function __sale_items_gst_report_download($rowData, $rowFooter, $sale_items) {
        ...
        if (!empty($rowData)) {
            ...
            $cellColoumnTitle = ['A' => 'Sr No',
                ...
                'Y' => "Product's Tax Amount",
                'Z' => 'Total Taxable Amount',
                'AA' => 'Invoice Tax Amount',
                'AB' => 'Total Amount',
                'AC' => 'Invoice Status',
                'AD' => 'Biller Name',
                'AE' => 'Warehouse',
            ...
            foreach ($rowData as $sale_id => $inv) {
                 ...
                 $total_taxable_amount += $inv[$cellData['Z']];
                 $toal_invoice_tax_amount  += $inv[$cellData['AA']];
                 $total_invoice_amount += $inv[$cellData['AB']];
               
                $sr++;
                $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sr);

                foreach (range('B', 'I') as $cell_1) {
                    $this->excel->getActiveSheet()->SetCellValue($cell_1 . $row, ( ($cell_1 == 'E')? date('Y-m-d',strtotime($inv[$cellData[$cell_1]])) :$inv[$cellData[$cell_1]]));
                }//end foreach

                foreach (range('X', 'Z') as $cell_3) {
                    $this->excel->getActiveSheet()->SetCellValue($cell_3 . $row, $inv[$cellData[$cell_3]]);
                }//end foreach

                foreach (range('A', 'B') as $cell_4) {
                    $this->excel->getActiveSheet()->SetCellValue('A' .$cell_4 . $row, $inv[$cellData['A'.$cell_4]]);
                }//end foreach

                foreach (range('C', 'L') as $cell_Ax) {
                    $this->excel->getActiveSheet()->SetCellValue('A'.$cell_Ax . $row, $inv[$cellData['A'.$cell_Ax]]);
                }//end foreach

                // $this->excel->getActiveSheet()->SetCellValue('AM'. $row , $inv[$cellData['AK']]);
                ...
                foreach ($inv['items'] as $item) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('K' . $tax_row, $item['product_name']);
                    $this->excel->getActiveSheet()->SetCellValue('L' . $tax_row, $item['product_code']);
                    $this->excel->getActiveSheet()->SetCellValue('M' . $tax_row, $item['option_name']);
                    $this->excel->getActiveSheet()->SetCellValue('N' . $tax_row, $item['quantity']);
                    $this->excel->getActiveSheet()->SetCellValue('O' . $tax_row, $item['net_unit_price']);
                    $this->excel->getActiveSheet()->SetCellValue('P' . $tax_row, $item['item_discount']);
                    $this->excel->getActiveSheet()->SetCellValue('Q' . $tax_row, $item['tax_rate']);
                    $this->excel->getActiveSheet()->SetCellValue('R' . $tax_row, $item['hsn_code']);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('S' . $tax_row, $cgst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('T' . $tax_row, $item['cgst']);
                    $this->excel->getActiveSheet()->SetCellValue('U' . $tax_row, $sgst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('V' . $tax_row, $item['sgst']);
                    $this->excel->getActiveSheet()->SetCellValue('W' . $tax_row, $igst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('X' . $tax_row, $item['igst']);
                    $this->excel->getActiveSheet()->SetCellValue('Y' . $tax_row, $item['tax_total']);
                    $this->excel->getActiveSheet()->SetCellValue('Z' . $tax_row, $item['taxable_amount']);
                ...
                $row++;
                $this->excel->getActiveSheet()->getStyle("A" . $row . ":AL" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
            }
             ...
             $this->excel->getActiveSheet()->SetCellValue('T' . $row , $total_cgst);
             $this->excel->getActiveSheet()->SetCellValue('V' . $row , $total_sgst);
             $this->excel->getActiveSheet()->SetCellValue('X' . $row , $total_igst);
            
            $this->excel->getActiveSheet()->SetCellValue('Y' . $row , $total_tax_amount);
            
            $this->excel->getActiveSheet()->SetCellValue('Z' . $row , $total_taxable_amount);
            $this->excel->getActiveSheet()->SetCellValue('AA' . $row , $toal_invoice_tax_amount);
            $this->excel->getActiveSheet()->SetCellValue('AB' . $row , $total_invoice_amount);
        ...
        if (!empty($rowData)) {
           ...
           // $this->excel->getActiveSheet()->getStyle("A" . $row . ":Y" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
            ...
            foreach ($rowData as $invoice_no => $inv) {
             ...
             $this->excel->getActiveSheet()->SetCellValue('L' . $row , $total_cgst);
             $this->excel->getActiveSheet()->SetCellValue('N' . $row , $total_sgst);
             $this->excel->getActiveSheet()->SetCellValue('P' . $row , $total_igst);
                
             
             $this->excel->getActiveSheet()->SetCellValue('Q' . $row , $total_tax_amount);
             $this->excel->getActiveSheet()->SetCellValue('R' . $row , $total_product_tax_amount);
             $this->excel->getActiveSheet()->SetCellValue('S' . $row , $total_invoice_tax_amount);
             $this->excel->getActiveSheet()->SetCellValue('T' . $row , $total_discount);
             $this->excel->getActiveSheet()->SetCellValue('U' . $row , $total_amount);
